...
void busSchedule(map < pair<int,string>, vector<string> >& m, string& s, const int& q){
    map < pair<int,string>, vector<string> >::iterator it;
    if(s=="NEW_BUS"){
        string bus, stop;
        int numStop;
        cin>>bus>>numStop;
...
{
    int Q;
    map < pair<int,string>, vector<string> >  m;
    cin>>Q;
    for(int i = 0; i < Q; i++){
        string command;
        cin>>command;
